...
- 启动时：加载 `zzauto.yaml`（不存在则用默认值）→ 创建工作区目录 → aiclibridge 健康检查（不可达则自动安装，除非 `--no-auto-install`）→ 装配编排器并注册 9 个 agent → 启动 HTTP 服务。
- 不带任何子命令（直接 `zzauto`）等同 `serve`。

成功启动后会看到：

```
zzauto v0.1.0 监听 127.0.0.1:8788
```
