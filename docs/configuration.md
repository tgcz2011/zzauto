...
| `role_models` | map[string]string | `{}` | `ZZAUTO_ROLE_MODEL_<STAGE>` | 每角色模型配置，key 为 stage 小写（listener/asker/planner/designer/evaluator/manager/executor/generator/gittor），value 为模型名；空字符串走默认模型 |

> `github` 为子结构，环境变量用 `ZZAUTO_GITHUB_*` 分别覆盖其字段。
> `role_models` 是 stage→model 名映射，未列出的 stage 走 aicli 默认模型（`claude/anthropic/claude-sonnet-4.5`）。详见 [settings.md](./settings.md)。
...
# 每角色模型配置（v0.3.0）
# key 为 stage 小写：listener/asker/planner/designer/evaluator/manager/executor/generator/gittor
# value 为模型名；空字符串走默认模型 claude/anthropic/claude-sonnet-4.5
role_models:
  listener: ""
  asker: ""
  planner: ""
...
| `ZZAUTO_ROLE_MODEL_<STAGE>` | `role_models[stage]` | key 由大写转小写，如 `ZZAUTO_ROLE_MODEL_LISTENER=foo` → `role_models["listener"]="foo"` |
...
| 文件 | 职责 |
| --- | --- |
| `internal/config/config.go` | 配置加载（含 `RoleModels` + `Save`）与默认值 |
| `internal/gittor/gittor.go` | token 安全处理 |
| `internal/ui/handler.go` | `/api/config`、`/api/github`（内存更新）、`/api/settings/models`（GET/PUT，PUT 时调 `cfg.Save` 落盘） |
| `internal/aicli/client.go` | `aicli_addr`/`aicli_key` 使用，`ChatWithModel`/`AskWithModel` 按角色模型调用 |
